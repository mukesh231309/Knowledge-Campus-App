$(document).ready(function(){
	var url="https://kitetech.ac.in/App/auth-kite.php?callback=?";
...
//update
    ...
    $("#add").click(function(){
		...
		var subject=$("#search-box").val(); //alert(subject);
		var topic=$("#topic").val(); //alert(topic);
		var subtopic=$("#sub-topic").val(); //alert(subtopic);
		var subtopic1=$("#sub-topic1").val(); //alert(subtopic1);
		var subtopic2=$("#sub-topic2").val();
	...
	//alert(username);
    	...
    	var dataString="time="+time+"&subject="+subject+"&topic="+topic+"&subtopic="+subtopic+"&subtopic1="+subtopic1+"&subtopic2="+subtopic2+"&semester="+semester+"&branch="+branch+"&activity="+activity+"&room="+room+"&des="+des+"&username="+username+"&total="+total+"&present="+present+"&batch="+batch+"&add=";

    	if($.trim(username).length>0 & $.trim(activity).length>0 & $.trim(room).length>0 )
		{
			$.ajax({
				type: "POST",
				url: url,
