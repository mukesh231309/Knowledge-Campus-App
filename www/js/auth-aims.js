$(document).ready(function(){
	var url="https://kitetech.ac.in/App/auth-aims.php?callback=?";
...
//update
    ...
    $("#add").click(function(){
    	...
    	var dataString="time="+time+"&semister="+semister+"&subject="+subject+"&topic="+topic+"&subtopic="+subtopic+"&activity="+activity+"&room="+room+"&des="+des+"&username="+username+"&add=";

    	if($.trim(username).length>0 & $.trim(activity).length>0 & $.trim(room).length>0 &  $.trim(des).length>0)
		{
			$.ajax({
				type: "POST",
				url: url,
