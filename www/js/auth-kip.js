$(document).ready(function(){
	var url="https://kitetech.ac.in/App/auth-kip.php?callback=?";
...
//update
    ...
    $("#add").click(function(){
    	...
    	var dataString="time="+time+"&class="+class1+"&activity="+activity+"&room="+room+"&des="+des+"&username="+username+"&add=";
    	...
    	if($.trim(username).length>0 & $.trim(activity).length>0 & $.trim(room).length>0 &  $.trim(des).length>0)
		{
			$.ajax({
				...
				success: function(data){
					...
					{
						alert("Submitted");
						window.location.href = "index-kip.html";
					}
					else if(data="failed")
					{
